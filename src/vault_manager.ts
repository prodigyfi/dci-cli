...
export class VaultManager {
  ...
  async createVault(createVaultOptions) {
    ...
    const listingFeeRateBN = parseUnits(
      this.basicSettings.listingFeeRate,
      fixDecimals,
    );
    ...
    if (isBuyLow) {
      ...
      investmentTokenAmount += listingFee;
    } else {
      linkedTokenAmount =
        (quantityBN * (fixDecimalsBigNumber + yieldValueBN) * linkedPriceBN) /
        fixDecimalsBigNumber /
        fixDecimalsBigNumber;
      linkedTokenAmount += listingFee;
    }
  ...
  async cancelVault(vaultAddress: string) {
    ...
    if (isBuyLow) {
      ...
      const oraclePriceAtCreation = BigInt(await vault.oraclePriceAtCreation());
      cancellationFee =
