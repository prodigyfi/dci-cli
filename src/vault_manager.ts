...
export class VaultManager {
  ...
  async createVault(createVaultOptions) {
    ...
    let tradingFeeRateBN = (await factory.getPresetFeeParams()).tradingFeeRate;

    const feeTiers: {
      minimumVolume: bigint;
      feeRate: bigint;
    }[] = await factory.getTradingFeeTiers();

    const volume = isBuyLow
      ? quantityBN
      : (quantityBN * oraclePriceAtCreationBN) / fixDecimalsBigNumber;

    feeTiers.forEach((feeTier) => {
      if (volume >= feeTier.minimumVolume) {
        tradingFeeRateBN = feeTier.feeRate;
      }
    });
    ...
    if (result.status == 1) {
      ...
      console.log(`Vault address: ${parsedLog.args.vaultAddress}`);
    }
  }
