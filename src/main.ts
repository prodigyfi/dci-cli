...
import { validHedges, validateHedge, hedgeWith } from "./hedge.ts";
...
async function main() {
  ...
  switch (commandOptions.command) {
    case "createVault": {
      ...
      if (
        !!createVaultOptions.hedge &&
        !validateHedge(createVaultOptions.hedge)
      ) {
        valid = false;
        console.error(
          "[INVALID HEDGE] Available hedges: " + validHedges().join(" "),
        );
      }
      ...
      if (createVaultOptions.hedge) {
        await hedgeWith(createVaultOptions);
      }
      break;
    }
    case "cancelVault": {
