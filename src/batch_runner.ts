...
const networkOptions = ["Base Testnet", "Base Mainnet", "Bera Testnet"];

const tradingPairOptions = ["WETH-USDC"];
...
async function createVaults(dataSet) {
  for (const [index, line] of dataSet.entries()) {
    ...
    if (!isToday(creationDate)) continue;
    ...
    const hedgeFlag = hedge ? "--hedge " + hedge : "";

    // TODO: the command line only works under POSIX-compliant systems; additional efforts are required for Win32
    const commandLine = `npx ts-node ./src/main.ts "${network}" createVault -t ${tradingPair} -p ${linkedPrice} -q ${quantity} -e ${exactExpiration} -y ${extractPercentage(yieldPercentage)} ${buyLowFlag} ${hedgeFlag} 2>&1 | tee -a ${LOG_FILE}`;
